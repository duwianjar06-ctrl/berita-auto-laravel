...
class ArticleQualityGate {
 public function check(Article $a, ?array $safety=null): array {
  $reasons=[];$score=0;$title=trim((string)$a->title);$excerpt=trim((string)$a->excerpt);$content=trim(strip_tags((string)$a->content));$source=trim((string)($a->source_content?:$a->content));
  ...
  if($a->publisher&&$a->category_id&&filter_var($a->source_url,FILTER_VALIDATE_URL)&&$a->source_published_at)$score+=10;else $reasons[]='metadata_incomplete';
