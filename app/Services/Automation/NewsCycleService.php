...
class NewsCycleService
{
    ...
    {
        if (!config('berita.automation_enabled')) {
            return ['status' => 'disabled'];
        }

        $lock = AutomationLock::firstOrCreate(['name' => 'news_cycle']);

        if ($lock->expires_at && $lock->expires_at->isFuture()) {
        ...
        try {
            $run = AutomationRun::create([
                ...
                'trigger' => 'scheduler',
                'status' => 'running',
                'target' => (int) config('berita.news_publish_target', 2),
                'started_at' => now(),
                'telemetry' => [],
            ]);

            $ing = app(NewsIngestionService::class)->run();
            ...
            $pub = app(ArticleGenerationService::class)->publishBatch();
