    ...
    <section aria-labelledby="featured-heading" class="border-b border-slate-200 pb-8">
        ...
        @if($articles->count())
            ...
            <div class="grid gap-7 lg:grid-cols-[minmax(0,1.6fr)_minmax(280px,.8fr)]">
                ...
                @php($supporting = $articles->slice(1, 3))
                @if($supporting->count())
                    <div class="divide-y divide-slate-200 border-t border-slate-200 lg:border-l lg:border-t-0 lg:pl-6">
                        @foreach($supporting as $article)
                            <article class="py-5 first:pt-0 lg:first:pt-0">
                                ...
                                <h3 class="mt-1 text-xl font-extrabold leading-snug">
                                    <a href="{{ route('article', $article->slug) }}" class="hover:underline decoration-2 underline-offset-4">{{ $article->title }}</a>
                                </h3>
                                @if($article->excerpt)
                ...
                @foreach($articles->slice(4) as $article)
                    <article class="grid gap-4 py-5 sm:grid-cols-[180px_minmax(0,1fr)]">
                        ...
                        <div class="min-w-0">
                            ...
                            <h3 class="mt-1 text-xl font-extrabold leading-snug sm:text-2xl">
                                <a href="{{ route('article', $article->slug) }}" class="hover:underline decoration-2 underline-offset-4">{{ $article->title }}</a>
                            </h3>
                            @if($article->excerpt)
