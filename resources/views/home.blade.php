    ...
    <section class="mt-10" aria-labelledby="latest-heading">
        <div class="flex items-end justify-between gap-4">
            <div>
                ...
                <h2 id="latest-heading" class="mt-1 text-2xl font-extrabold tracking-tight">Berita Terbaru</h2>
            </div>
                ...
                <article class="group overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 transition hover:-translate-y-0.5 hover:shadow-md">
                    @if($article->image_url)
                        <a href="{{ route('article', $article->slug) }}" class="block aspect-[16/9] overflow-hidden bg-slate-100">
                            <img loading="lazy" src="{{ $article->image_url }}" alt="{{ $article->image_alt ?: $article->title }}" class="h-full w-full object-cover transition duration-300 group-hover:scale-[1.02]">
                        </a>
                    @else
                    ...
                    <div class="p-5">
                        ...
                        <h3 class="mt-2 text-lg font-extrabold leading-snug">
                            <a class="group-hover:underline" href="{{ route('article', $article->slug) }}">{{ $article->title }}</a>
                        </h3>
                        @if($article->excerpt)
            ...
            <div class="mt-5 flex flex-wrap gap-3">
                @foreach($categories as $category)
                    <a href="{{ route('category', $category->name) }}" class="rounded-full bg-white px-4 py-2 text-sm font-semibold ring-1 ring-slate-200 hover:bg-slate-900 hover:text-white">
                        {{ $category->name }} <span class="ml-1 text-slate-400">{{ $category->articles_count }}</span>
                    </a>
                @endforeach
            </div>
        </section>
