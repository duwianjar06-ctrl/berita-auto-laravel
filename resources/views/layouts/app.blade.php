...
<head>
    ...
    <style>:root{color-scheme:light}html{scroll-behavior:smooth}body{margin:0;background:#f8fafc;color:#111827;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}[x-cloak]{display:none!important}.article-copy p{margin:1.25rem 0 0;line-height:1.8}.article-copy p:first-child{margin-top:0}.article-copy a{text-decoration:underline;text-underline-offset:3px}:focus-visible{outline:2px solid #101a33;outline-offset:3px}</style>
    @stack('head')
</head>
<body class="min-h-screen antialiased">
    <header class="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur" x-data="{open:false}">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="flex min-h-16 items-center justify-between gap-4">
                ...
                <button type="button" class="grid h-10 w-10 shrink-0 place-items-center border border-slate-300 text-slate-800 hover:border-slate-900 md:hidden" x-on:click="open=!open" :aria-expanded="open.toString()" aria-controls="mobile-navigation" aria-label="Buka navigasi">
    ...
    <footer class="mt-14 border-t border-slate-300 bg-white">
        <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
            <div class="grid gap-8 md:grid-cols-[1.4fr_2fr]">
                <div>
                    <p class="text-lg font-black text-slate-950">Berita Auto</p>
                    <p class="mt-2 max-w-sm text-sm leading-6 text-slate-500">Portal berita dengan sumber yang dapat ditelusuri, proses editorial, dan quality gate sebelum publikasi.</p>
                </div>
                <nav class="grid grid-cols-2 gap-x-6 gap-y-3 text-sm font-semibold text-slate-600 sm:grid-cols-3" aria-label="Navigasi footer">
                    <a class="hover:text-slate-950" href="{{ route('about') }}">Tentang Kami</a>
                    <a class="hover:text-slate-950" href="{{ route('contact') }}">Kontak</a>
                    <a class="hover:text-slate-950" href="{{ route('privacy') }}">Kebijakan Privasi</a>
                    <a class="hover:text-slate-950" href="{{ route('disclaimer') }}">Disclaimer</a>
                    <a class="hover:text-slate-950" href="{{ route('editorial.policy') }}">Editorial Policy</a>
                    <a class="hover:text-slate-950" href="{{ route('sitemap') }}">Sitemap</a>
                </nav>
            </div>
            <div class="mt-8 border-t border-slate-200 pt-5 text-xs text-slate-500">© {{ date('Y') }} Berita Auto</div>
        </div>
    </footer>
