    ...
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex min-h-16 items-center justify-between gap-4">
            ...
            <button type="button" class="grid h-10 w-10 shrink-0 place-items-center border border-slate-300 text-slate-800 hover:border-slate-900 md:hidden" @click="open=!open" :aria-expanded="open.toString()" aria-controls="mobile-navigation" aria-label="Buka navigasi">
                <span class="sr-only">Menu</span><span class="block w-5"><span class="block h-0.5 bg-current"></span><span class="mt-1.5 block h-0.5 bg-current"></span><span class="mt-1.5 block h-0.5 bg-current"></span></span>
            </button>
        </div>
        <nav id="mobile-navigation" x-show="open" x-cloak class="border-t border-slate-200 py-3 md:hidden" aria-label="Navigasi mobile">
            <div class="grid gap-1 pb-1 text-sm font-bold"><a class="px-2 py-2 hover:bg-slate-50" href="{{ route('home') }}">Beranda</a>@if(isset($categories))@foreach($categories->take(8) as $category)<a class="px-2 py-2 hover:bg-slate-50" href="{{ route('category', $category->name) }}">{{ $category->name }}</a>@endforeach@endif</div>
        </nav>
    </div>
</header>
@if(isset($categories) && $categories->count())
<div class="border-b border-slate-200 bg-white"><nav class="mx-auto flex max-w-7xl gap-5 overflow-x-auto px-4 py-2.5 sm:px-6 lg:px-8" aria-label="Topik berita"><a href="{{ route('home') }}" class="shrink-0 text-xs font-bold uppercase tracking-wide text-slate-700 hover:text-slate-950">Semua</a>@foreach($categories as $category)<a href="{{ route('category', $category->name) }}" class="shrink-0 text-xs font-bold uppercase tracking-wide text-slate-500 hover:text-slate-950">{{ $category->name }}</a>@endforeach</nav></div>
@endif
<main class="mx-auto max-w-7xl px-4 py-7 sm:px-6 sm:py-9 lg:px-8">@yield('content')</main>
<footer class="mt-10 border-t border-slate-300 bg-white"><div class="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-8 text-sm text-slate-500 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8"><p>© {{ date('Y') }} Berita Auto</p><nav class="flex gap-5" aria-label="Navigasi footer"><a class="font-semibold hover:text-slate-950" href="{{ route('home') }}">Beranda</a><a class="font-semibold hover:text-slate-950" href="{{ route('sitemap') }}">Sitemap</a></nav></div></footer>
